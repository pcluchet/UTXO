...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n ${CHAINCODE} -l ${LANGUAGE} -v ${VERSION} -c '{"Args":[]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
		...
		res=$?
                ...
                set -x
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE} -l ${LANGUAGE} -v 1.0 -c '{"Args":[]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
...
upgradeChaincode () {
    ...
    set -x
    peer chaincode upgrade -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE} -v 2.0 -c '{"Args":[]}' -P "OR ('Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')"
    res=$?
	set +x
    cat log.txt
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n ${CHAINCODE} -c '{"Args":["invoke","a","b","10"]}' >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE} -c '{"Args":["invoke","a","b","10"]}' >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
